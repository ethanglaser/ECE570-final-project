...
import numpy as np
from utils_amazon_tf2 import load_amazon, split_data, xavier_init, MLP, identity, turn_tfidf, turn_one_hot
...
class SourceOnly(object):
    def __init__(self, source_domain=0, target_domain=3, **kwargs):
        ...
        self.data_load_from = 'amazon.mat'
        self.batch_size = 200
        self.model_save_to = 'outputFull/SourceOnly_{0}_to_{1}.pkl'.format(source_domain, target_domain)
    ...
    def build_model(self):
        ...
        self.Y = tf.compat.v1.placeholder("float", [None, n_classes])
        ...
        self.accuracy = tf.reduce_mean(input_tensor=tf.cast(tf.equal(tf.argmax(input=self.pred, axis=1),
                                                        tf.argmax(input=self.Y, axis=1)), 'float'))
        self.C_loss = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=self.pred,
                                                                             labels=tf.stop_gradient(self.Y)))
        l2_norm = 0.
        for tensor in list(self.weights.values()):
            l2_norm += tf.reduce_sum(input_tensor=tf.abs(tensor))
    ...
    def train(self, x_train, y_train, x_valid, y_valid, x_test, y_test, x_s_tst, x_t_tst):
        ...
        with tf.GradientTape() as tape:
            ...
            while True:
                X = x_train
                Y = y_train
                weights = {'h1': tf.Variable(tf.cast(xavier_init([self.n_input, self.n_hidden_1]),tf.float64)), 'out': tf.Variable(tf.cast(xavier_init([self.n_hidden_1, self.n_classes]), tf.float64)),}
                biases = {'b1': tf.Variable(tf.cast(tf.zeros(shape=[self.n_hidden_1]), tf.float64)), 'out': tf.Variable(tf.cast(tf.zeros(shape=[self.n_classes]), tf.float64)),}
                theta = list(weights.values()) + list(biases.values())
                encoding = tf.nn.sigmoid(tf.add(tf.matmul(X, weights['h1']), biases['b1']))
                pred = tf.matmul(encoding, weights['out']) + biases['out']
                grads = tf.gradients(pred, theta)
                accuracy = tf.reduce_mean(input_tensor=tf.cast(tf.equal(tf.argmax(input=pred, axis=1), tf.argmax(input=Y, axis=1)), 'float'))
                C_loss = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=tf.stop_gradient(Y)))
                l2_norm = 0.
                for tensor in list(weights.values()):
                    l2_norm += tf.reduce_sum(input_tensor=tf.abs(tensor))
                loss = C_loss + 0.0001 * l2_norm
                grad = tape.gradient(pred, theta)
                loss_fn = lambda: loss
                solver = optimizer.minimize(loss_fn, theta)
                Y = y_valid
                X = x_valid
                encoding = tf.nn.sigmoid(tf.add(tf.matmul(X, weights['h1']), biases['b1']))
                pred = tf.matmul(encoding, weights['out']) + biases['out']
                valid_accuracy = tf.reduce_mean(input_tensor=tf.cast(tf.equal(tf.argmax(input=pred, axis=1), tf.argmax(input=Y, axis=1)), 'float'))
                ...
                if wait_times >= self.tolerate_time:
                    break
            encoding_s = tf.nn.sigmoid(tf.add(tf.matmul(x_s_tst, weights['h1']), biases['b1']))
            encoding_t = tf.nn.sigmoid(tf.add(tf.matmul(x_t_tst, weights['h1']), biases['b1']))
...
class SCMD(object):
    def __init__(self, source_domain=0, target_domain=3, **kwargs):
        ...
        self.data_load_from = 'amazon.mat'
        self.batch_size = 200
        self.model_save_to = 'output_tf2/SCMD_{0}_to_{1}.pkl'.format(source_domain, target_domain)
        self.model_load_from = self.model_save_to
        self.n_input = 5000
        self.n_classes = 2
        self.model_built = False
    ...
    def build_model(self):
        ...
        def matchnorm(x1, x2):
            return tf.sqrt(tf.reduce_sum(input_tensor=tf.pow(x1 - x2, 2)))

        def scm(sx1, sx2, k):
            ss1 = tf.reduce_mean(input_tensor=tf.pow(sx1, k), axis=0)
            ss2 = tf.reduce_mean(input_tensor=tf.pow(sx2, k), axis=0)
            return matchnorm(ss1, ss2)

        def mmatch(x1, x2, n_moments):
            mx1 = tf.reduce_mean(input_tensor=x1, axis=0)
            mx2 = tf.reduce_mean(input_tensor=x2, axis=0)
            sx1 = x1 - mx1
            sx2 = x2 - mx2
        ...
        self.R_loss = tf.reduce_mean(input_tensor=tf.square(decoding_t_u - self.X_t_u))
        ...
        self.C_loss = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=pred_s, labels=tf.stop_gradient(self.Y_s)))
        correct_prediction = tf.equal(tf.argmax(input=pred_s, axis=1), tf.argmax(input=self.Y_s, axis=1))
        self.accuracy_s = tf.reduce_mean(input_tensor=tf.cast(correct_prediction, "float"))
        # Build solver
...
class DCMD(SCMD):
    def __init__(self, source_domain=0, target_domain=3, **kwargs):
        ...
        self.model_save_to = 'output_tf2/DCMD_{0}_to_{1}.pkl'.format(source_domain, target_domain)
        self.lamb = 1.
    ...
    def build_model(self):
        ...
        def get_correlation(mat_a, mat_b):
            mat_a = mat_a - tf.reduce_mean(input_tensor=mat_a, axis=0)
            mat_b = mat_b - tf.reduce_mean(input_tensor=mat_b, axis=0)
            sigma = tf.matmul(tf.transpose(a=mat_a), mat_b)
            mat_a_cov = tf.matmul(tf.transpose(a=mat_a), mat_a)
            mat_b_cov = tf.matmul(tf.transpose(a=mat_b), mat_b)
            return tf.matmul(tf.matmul(tf.linalg.tensor_diag(tf.pow(tf.linalg.tensor_diag_part(mat_a_cov), -0.5)), sigma),
                             tf.linalg.tensor_diag(tf.pow(tf.linalg.tensor_diag_part(mat_b_cov), -0.5)))
        ...
        self.corr_loss = tf.reduce_mean(input_tensor=tf.abs(get_correlation(encoding_c_t_u, encoding_t_t_u)))
        # Get reconstruction loss
        decoding_c_t_u = self.common_decode_mlp.apply(encoding_c_t_u)
        decoding_t_t_u = self.target_decode_mlp.apply(encoding_t_t_u)
        decoding_t_u = decoding_c_t_u + decoding_t_t_u
...
class TransferClassifier(object):
    ...
    def __init__(self, source_domain=0, target_domain=3, **kwargs):
        ...
        self.learning_rate = 5e-3
        self.data_load_from = 'amazon.mat'
        self.model_save_to = 'outputFull/CMD_{0}_to_{1}.pkl'.format(source_domain, target_domain)
        self.model_load_from = self.model_save_to
        self.n_input = 5000
        self.n_hidden_1 = 50
        self.n_classes = 2
    ...
    def build_model(self):
        ...
        self.Y_s = tf.compat.v1.placeholder("float", [None, self.n_classes])
        self.Y_t = tf.compat.v1.placeholder("float", [None, self.n_classes])
        self.X_s_u = tf.compat.v1.placeholder("float", [None, self.n_input])
        self.X_t_u = tf.compat.v1.placeholder("float", [None, self.n_input])
        ...
        self.accuracy_s = tf.reduce_mean(input_tensor=tf.cast(tf.equal(tf.argmax(input=self.pred_s, axis=1),
                                                          tf.argmax(input=self.Y_s, axis=1)), 'float'))
        ...
        self.accuracy_t = tf.reduce_mean(input_tensor=tf.cast(tf.equal(tf.argmax(input=self.pred_t, axis=1),
                                                          tf.argmax(input=self.Y_t, axis=1)), 'float'))

        self.C_loss = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=self.pred_s,
                                                                             labels=tf.stop_gradient(self.Y_s)))
        ...
        self.l2_norm = 0.
        for tensor in list(self.weights.values()):
            self.l2_norm += tf.reduce_sum(input_tensor=tf.abs(tensor))
        self.loss = self.C_loss + self.alpha * self.D_loss + 0.0001*self.l2_norm
        self.solver = (tf.compat.v1.train.RMSPropOptimizer(learning_rate=self.learning_rate)
                       .minimize(self.loss, var_list=self.theta))
        self.C_solver = (tf.compat.v1.train.RMSPropOptimizer(learning_rate=self.learning_rate)
...
class CoTrainer(object):
    def __init__(self, source_domain=0, target_domain=3, **kwargs):
        ...
        self.data_load_from = 'amazon.mat'
        self.batch_size = 200
        self.model_save_to = 'output_tf2/Cotrain_{0}_to_{1}.pkl'.format(source_domain, target_domain)
        self.model_load_from = self.model_save_to
        self.common_model_save_to = 'output_tf2/Cotrain_common_{0}_to_{1}.pkl'.format(source_domain, target_domain)
        self.common_tune_model_save_to = 'output_tf2/Cotrain_common_tune_{0}_to_{1}.pkl'.format(source_domain, target_domain)
        self.target_model_save_to = 'output_tf2/Cotrain_target_{0}_to_{1}.pkl'.format(source_domain, target_domain)
        self.combined_model_save_to = 'output_tf2/Cotrain_combined_{0}_to_{1}.pkl'.format(source_domain, target_domain)
        self.n_input = 5000
        self.n_classes = 2
        self.d_hidden = 50
        self.tolerate_time = 20
    ...
    def build_model(self):
        ...
        self.T_loss = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=pred_t, labels=tf.stop_gradient(self.Y_t)))
        correct_prediction = tf.equal(tf.argmax(input=pred_t, axis=1), tf.argmax(input=self.Y_t, axis=1))
        self.accuracy_t = tf.reduce_mean(input_tensor=tf.cast(correct_prediction, "float"))
        # Build solver
        self.theta = (self.common_encode_mlp.parameters +
                      self.target_encode_mlp.parameters +
                      self.common_decode_mlp.parameters +
        ...
        for tensor in self.target_output_mlp.parameters:
            if tensor.name.find('W') != 0:
                l2_norm += 4 * tf.reduce_sum(input_tensor=tf.abs(tensor))
        self.loss = (self.R_loss +
                     # self.alpha * self.C_loss +
    ...
    def train_common_model(self, x_s_u, x_t_u, x_t, y_t, x_valid, y_valid, x_test, y_test):
        ...
        with self.graph.as_default():
            self.build_model()
            saver = tf.compat.v1.train.Saver(var_list=self.target_theta)
            self.sess.run(tf.compat.v1.global_variables_initializer())
    ...
    def train_target_model(self, x_s_u, x_t_u, x_t, y_t, x_valid, y_valid, x_test, y_test):
        ...
        with self.graph.as_default():
            self.build_model()
            saver = tf.compat.v1.train.Saver(var_list=self.common_theta)
            self.sess.run(tf.compat.v1.global_variables_initializer())
            saver.restore(self.sess, self.common_model_save_to)
            saver = tf.compat.v1.train.Saver(var_list=self.theta)
            while True:
    ...
    def analysis(self, x_t_train, y_t_train):
        ...
        with self.graph.as_default():
            ...
            saver = tf.compat.v1.train.Saver(var_list=self.theta)
            self.sess.run(tf.compat.v1.global_variables_initializer())
            saver.restore(self.sess, 'outputFull/combined.pkl')
...
class Autoencoder(object):
    def __init__(self, source_domain=0, target_domain=3, **kwargs):
        ...
        self.data_load_from = 'amazon.mat'
        self.batch_size = 200
        self.model_save_to = 'output_tf2/Auto_{0}_to_{1}.pkl'.format(source_domain, target_domain)
        self.model_load_from = self.model_save_to
        self.n_input = 5000
        self.n_classes = 2
        self.model_built = False
    ...
    def build_model(self):
        ...
        self.R_loss = tf.reduce_mean(input_tensor=tf.square(decoding_t_u - self.X_t_u)) + tf.reduce_mean(input_tensor=tf.square(decoding_s_u - self.X_s_u))
