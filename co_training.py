...
class CoTrainer(object):
    def __init__(self, source_domain=0, target_domain=3, **kwargs):
        ...
        self.batch_size = 200
        self.model_save_to = 'output/Cotrain_{0}_to_{1}.pkl'.format(source_domain, target_domain)
        self.model_load_from = self.model_save_to
        self.common_model_save_to = 'output/Cotrain_common_{0}_to_{1}.pkl'.format(source_domain, target_domain)
        self.target_model_save_to = 'output/Cotrain_target_{0}_to_{1}.pkl'.format(source_domain, target_domain)
        self.combined_model_save_to = 'output/Cotrain_combined_{0}_to_{1}.pkl'.format(source_domain, target_domain)
        self.n_input = 5000
        self.n_classes = 2
        self.d_hidden = 50
    ...
    def analysis(self, x_t_train, y_t_train):
        ...
        with self.graph.as_default():
            ...
            saver = tf.compat.v1.train.Saver(var_list=self.theta)
            self.sess.run(tf.compat.v1.global_variables_initializer())
            saver.restore(self.sess, 'output/combined.pkl')
...
class CoDSN(object):
    def __init__(self, source_domain=0, target_domain=3, **kwargs):
        ...
        self.model_save_to = 'output/Cotrain_{0}_to_{1}.pkl'.format(source_domain, target_domain)
        self.model_load_from = self.model_save_to
        self.common_model_save_to = 'output/Cotrain_common_{0}_to_{1}.pkl'.format(source_domain,
                                                                                                   target_domain)
        self.common_tune_model_save_to = 'output/Cotrain_common_tune_{0}_to_{1}.pkl'.format(
            source_domain, target_domain)
        self.target_model_save_to = 'output/Cotrain_target_{0}_to_{1}.pkl'.format(source_domain,
                                                                                                   target_domain)
        self.combined_model_save_to = 'output/Cotrain_combined_{0}_to_{1}.pkl'.format(source_domain,
                                                                                                       target_domain)
        self.n_input = 5000
        self.n_classes = 2
        self.d_hidden = 50
...
if __name__ == '__main__':
    ...
    print('Reslut from {0} domain to {1} doamin:{2}'.format(source_domain, target_domain, results[-1]))